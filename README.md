...
Clang static analyzer（以下简称CSA）：CSA 是一个基于Clang的c/c++/object-c源代码分析框架，它首先对源文件进行预处理得到关于某个模块的完整的控制流程图（CFG），然后可以支持使用符号执行来遍历整个源文件。开发者可以开发自己的插件，通过hook的方式在符号执行过程中与框架进行交互。具体地，在CSA中，所有的值都是一个符号（如函数返回值，局部变量，全局变量等），通过符号执行，每条执行路径上都对应着某个符号不同的约束范围，当静态分析运行到程序某个点上，开发者可利用框架提供的hook接口与符号执行引擎进行交互（例如在函数调用后询问该函数的返回值对应的符号值，该函数的原型如参数个数，参数类型，返回值类型等）。由于CSA本身提供了大量的api接口，开发者可在不同位置的hook接入，并利用这些接口得到大量有用的信息，进而可以分析和发现潜在的安全问题。

 利用CSA框架可以开展许多有趣的源代码分析，CSA本身基于Clang前端的强大代码分析处理能力，对现有代码的支持相当完善，这也是其主要的优势之一。接下来我们将尝试使用CSA框架来检测密码学库中的一些经典的密码学误用，这是一个很有意思的方向。另外 ，对于TripleDoggy项目中的三个checker，其实现尚有许多不足之处，我们也会陆续进行完善改进（例如由于符号执行原生的缺陷，即路径爆炸和约束求解性能差等还有待继续探索）。
